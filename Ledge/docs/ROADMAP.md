...
- [x] **Panel loses fullscreen when settings window is closed** — eliminated entirely. App now stays as `.accessory` permanently — no activation policy toggling at all. The previous `.regular` → `.accessory` toggle destabilised the fullscreen helper and caused macOS to maximize the frontmost main-display window. Menu bar icon provides settings access
- [x] **Panel position bug on display config change** — `handleDisplayChange()` now tears down and rebuilds the fullscreen helper when the Xeneon Edge screen reference changes, ensuring the fullscreen Space follows the display move
- [x] **Background mode override** — "Theme Color" mode now always shows the theme's solid `dashboardBackground` colour, regardless of widget background style. Widget style (solid/blur/transparent) is decoupled from the dashboard background choice
...
- [x] **Spotify marquee text transitions** — replaced fade-out/fade-in with coordinated wipe transitions: old text wipes out L→R (conceal sweep, 0.6s), album art crossfades mid-wipe (delayed 0.3s, 0.8s), new text wipes in L→R (reveal sweep, 0.8s), then gentle bounce scroll for overflow. Removed the disjointed simultaneous fade/marquee behaviour
- [x] **Spotify widget — radial album art bleed** — replaced radial-from-centre glow with 4 directional `LinearGradient`s from widget edges inward, using edge-sampled album art colours at near-full saturation (80%). `AlbumColorExtractor` now samples top/bottom/left/right edges separately. Adaptive text contrast: text colour switches to dark on bright album art (luminance > 0.65), with a subtle shadow safety net for mid-luminance backgrounds
- [x] **DateTime widget — timezone search fixed** — replaced broken custom TextField search with a manual search field inside the List + NavigationStack for proper macOS sheet sizing. Timezone rows now use a two-line layout (city name + ID) with vertical padding and drag-to-reorder support via `.onMove`
- [x] **DateTime widget — multiple styles** — four configurable display modes: **Digital** (default, monospaced thin), **Minimal** (large bold rounded time only), **Analog** (Canvas-drawn clock face with hour/minute/second hands and tick marks), **Retro** (heavy monospaced green-on-dark LED style). Style picker in settings, with conditional setting visibility per style
...
- [ ] **Launch visualization — water settling effect** — `LaunchWaveOverlay` plays a one-time damped sine-wave animation when the dashboard appears. Four horizontal wave bands oscillate with decreasing amplitude over 1.5s (`.easeOut`), drawn via `Canvas` at low opacity (6%). Non-interactive (`allowsHitTesting(false)`)
- [x] **Periodic visual effects** — replaced single shimmer with 3 selectable effects in Settings > Appearance > Visual Effects: **Light Wave** (gradient band sweeping at random angles), **Water Drop** (concentric ripple with blur halos expanding from random point, simulating liquid distortion), **Twinkle** (sparkle flashes along random widget edges). Configurable interval (30s–5min). `VisualEffectMode` enum in ThemeManager with migration from old `shimmerEnabled` setting. All effects use `.allowsHitTesting(false)` and trigger immediately when enabled
- [x] **Widget selector previews — verified safe** — confirmed the widget gallery renders live widgets via `descriptor.viewFactory()`. Calendar event titles ARE visible in previews. Accepted as intended behaviour since the user is already authenticated and the preview is in their own Settings window. Future improvement: add a `previewMode` environment flag for sample data
- [x] **Spotify widget — album art background styles** — five selectable styles: **None**, **Edge Fade** (existing directional bleed), **Frosted Glass** (blurred album art + dark overlay), **Vinyl Aura** (radial gradient from art centre), **Color Bands** (vertical gradient from extracted colours). Config migrated from `showAlbumColors: Bool` to `albumArtStyle: AlbumArtStyle` with backward-compatible JSON decoding. `AlbumColorExtractor` extended with `averageColor` for Frosted Glass tint
...
- [ ] **Pre-meeting warming pulse** — 10 minutes before a meeting is due to start, the calendar widget begins a slow pulsing animation in the colour of the calendar the meeting belongs to (from `EKCalendar.cgColor`). A warm, breathing pulse effect — opacity/scale cycling on a ~2–3s period. Pulse rate and intensity increase as the meeting approaches (gentle at 10min, urgent at 1min). Tap to dismiss or join (if meeting link detected). Resets when the meeting starts
...
- [ ] **Alarms & Timers widget** — visual alarm/timer alerts on the Edge panel. Investigate macOS integration paths: (a) observe the built-in Clock/Timer app via `DistributedNotificationCenter` or Shortcuts actions, (b) built-in timer functionality within Ledge itself (set countdown timers directly on the Edge), (c) Reminders/Calendar-based alarms. When an alarm fires, trigger a full-width visual alert on the panel — flashing colour wash, pulsing glow, or animated banner — that demands attention without producing sound. Tap to dismiss. Could also support recurring alarms (e.g., daily standup reminder) and Pomodoro-style work/break cycles
