...
- [x] **Panel loses fullscreen when settings window is closed** — `updateActivationPolicy()` now calls `displayManager.reassertFullscreen(on:)` after switching to `.accessory`, which checks the fullscreen helper state and rebuilds it if needed
...
- [ ] **DateTime widget — timezone search broken (BUG)** — the timezone/location picker in the DateTime widget configuration does not search properly and never shows any matching results. Users are unable to add additional timezones. Investigate the search/filter logic in the timezone selection UI — likely a binding or predicate issue preventing results from appearing
